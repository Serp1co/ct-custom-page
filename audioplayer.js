class FadeInLoop {
  ...
  async stop() {
    const now = this.ctx.currentTime;
    this.gainNode.gain.setValueAtTime(Number.EPSILON, now);
    this.gainNode.gain.exponentialRampToValueAtTime(0, now + 0.155);
...
const first_loop = new FadeInLoop(
  ctx,
  "https://audio.jukehost.co.uk/jdWaU4FggWzEnDzqrXbwVt5p1ZByzlqq"
);
const second_loop = new FadeInLoop(
  ctx,
  "https://audio.jukehost.co.uk/jdWaU4FggWzEnDzqrXbwVt5p1ZByzlqq"
);
const third_loop = new FadeInLoop(
  ctx,
  "https://audio.jukehost.co.uk/jdWaU4FggWzEnDzqrXbwVt5p1ZByzlqq"
);
